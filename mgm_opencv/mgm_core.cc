...
inline void update_cost2_trunclinear(Dvec &Lp, Dvec &CCp, Dvec &Lq, Dvec &Lr, const float P1, const float P2) {
	...
	float* M1 = new float[Lp.max - Lp.min + 1];
	float* M2 = new float[Lp.max - Lp.min + 1];
	int mm = Lp.min;
...
inline void update_costW_trunclinear(Dvec &Lp, Dvec &CCp, Dvec &Lq, Dvec &Lr, Dvec &Ls, Dvec &Lt, const float P1, const float P2,
	...
	float* M1 = new float[NN];
	float* M2 = new float[NN];
	float* M3 = new float[NN];
	float* M4 = new float[NN];
...
inline bool blayer_condition(Dvec& Lp, Dvec& CCp, const cv::Mat& occl_blayer, int pridx, int pr2idx, int pr3idx, int pr4idx, int MGM) {
	int howMuch = 0;
	if ((occl_blayer.data)[pridx] == 255){
		howMuch++;
		if ((occl_blayer.data)[pr2idx] == 255){
			howMuch++;
			if ((occl_blayer.data)[pr3idx] == 255){
				howMuch++;
				if ((occl_blayer.data)[pr4idx] == 255){
					howMuch++;
				}
			}
		}
...
struct costvolume_t mgm(struct costvolume_t CC, const cv::Mat& in_w,
	...
	int SGM_FIX_OVERCOUNT = 1, const cv::Mat* occl_blayer = NULL, int LET_OCCL_PROPAGATE = 0)                 // fix the overcounting in SGM following (Drory etal. 2014)
{

	int nx = dminI.cols;
	int ny = dminI.rows;
	float* wdata = (float*)in_w.data;

	// check the content of in_w is it all 1?
	int USE_IMAGE_DEPENDENT_WEIGHTS = 0;
	for (int i = 0; i < in_w.cols*in_w.rows*in_w.channels(); i++)
	if (wdata[i] != 1.0) USE_IMAGE_DEPENDENT_WEIGHTS = 1;
	...
	struct costvolume_t S = allocate_costvolume(dminI, dmaxI, maskdet);
	...
	// weights are inside of the image's w channels'
...
#pragma omp parallel for
		for (int pidx = 0; pidx < nx*ny; pidx++){
			if ((maskdet.data)[pidx] == MASK_OUT){
				continue;
			}
			for (int o = Lr[pidx].min; o <= Lr[pidx].max; o++){
				Lr[pidx].set_nolock(o, CC[pidx][o]); // no omp critic is inside set_nolock ??? todo ref or val
			}
		}
...
#define SWAPi(a,b) {int swap=a;a=b;b=swap;}
				...
				if (!check_inside_mask(p, maskdet)) continue;
				...
				if (!check_inside_image(pr, dminI) || !check_inside_mask(pr, maskdet)) continue;
				if (!check_inside_image(pr2, dminI) || !check_inside_mask(pr2, maskdet)) continue;
				if (!check_inside_image(pr3, dminI) || !check_inside_mask(pr3, maskdet)) continue;
				if (!check_inside_image(pr4, dminI) || !check_inside_mask(pr4, maskdet)) continue;


				if (LET_OCCL_PROPAGATE == 0 && occl_blayer != NULL && blayer_condition(Lr[pidx], CC[pidx], *occl_blayer, pridx, pr2idx, pr3idx, pr4idx, MGM)){
					continue;
				}
				...
				if (TSGM_2LMIN > 0) { // THIS IS A LEGACY FEATURE
					...
					int channelsWeights = 8;
#define valof(u, d, p, ch, chWeights)  d[(ch) + ((int)p.x)*(chWeights) + (u.cols)*((int)p.y)*(chWeights)]
					float DeltaI1 = valof(in_w, wdata, p, pass_to_channel_1[pass], channelsWeights);
					float DeltaI2 = valof(in_w, wdata, p, pass_to_channel_2[pass], channelsWeights);
					float DeltaI3 = valof(in_w, wdata, p, pass_to_channel_3[pass], channelsWeights);
					float DeltaI4 = valof(in_w, wdata, p, pass_to_channel_4[pass], channelsWeights);
#undef valof
		...
		for (int i = 0; i < nx*ny; i++) {
			if ((maskdet.data)[i] == MASK_OUT){
				for (int o = Lr[i].min; o <= Lr[i].max; o++) {
					S[i].set_nolock(o, INFINITY);
				}
				continue;
			}
			for (int o = Lr[i].min; o <= Lr[i].max; o++) {
				S[i].increment_nolock(o, Lr[i][o]); // no pragma omp critic is inside set
			}
		}
	}


	float* dataout = (float*)out.data;
	float* dataoutcost = (float*)outcost.data;
	// WTA 
#pragma omp parallel for
	for (int i = 0; i<nx*ny; i++) {
		float minP = std::numeric_limits<float>::quiet_NaN();
		float minL = INFINITY;
		...
		dataout[i] = minP; // disparities
		dataoutcost[i] = minL; // costs for the disparity above
	}
...
struct costvolume_t mgm_naive_parallelism(struct costvolume_t CC, const cv::Mat& in_w,
	const cv::Mat& dminI, const cv::Mat& dmaxI,
	cv::Mat& out, cv::Mat& outcost, const cv::Mat& maskdet,
	const float P1, const float P2, const int NDIR, const int MGM,
	const int USE_FELZENSZWALB_POTENTIALS = 0, // USE SGM(0) or FELZENSZWALB(1) POTENTIALS
	int SGM_FIX_OVERCOUNT = 1, const cv::Mat* occl_blayer = NULL, int LET_OCCL_PROPAGATE = 0)                 // fix the overcounting in SGM following (Drory etal. 2014)
{
	...
	// check the content of in_w is it all 1?
	int USE_IMAGE_DEPENDENT_WEIGHTS = 0;
	float* in_wdata = (float*)in_w.data;
	for (int i = 0; i < in_w.cols*in_w.rows*in_w.channels(); i++)
	if (in_wdata[i] != 1.0) USE_IMAGE_DEPENDENT_WEIGHTS = 1;
	if (USE_IMAGE_DEPENDENT_WEIGHTS) printf(" USING IMAGE DEPENDENT WEIGHTS\n");
...
#define SWAPi(a,b) {int swap=a;a=b;b=swap;}
				...
				if (LET_OCCL_PROPAGATE == 0 && occl_blayer != NULL && blayer_condition(Lr[pidx], CC[pidx], *occl_blayer, pridx, pr2idx, pr3idx, pr4idx, MGM)){
					for (int o = Lr[pidx].min; o <= Lr[pidx].max; o++) {
						Lr[pidx].set_nolock(o, CC[pidx][o]);
					}
					continue;
				}
